...
- Package: scop
- Language: R
- Function: `scVI_integrate`
- Source: https://mengxu98.github.io/scop/reference/scVI_integrate.html
- Raw source: https://raw.githubusercontent.com/mengxu98/scop/main/man/scVI_integrate.Rd
...
- `scVI_dims_use`: A vector specifying the dimensions returned by scVI that will be utilized for downstream cell cluster finding and nonlinear reduction. If set to NULL, all the returned dimensions will be used by default.
...
- `model`: A string indicating the scVI model to be used. Options are "SCVI", "PEAKVI", and "POISSONVI". Default is `"SCVI"`.
- `SCVI_params`: A list of parameters for the SCVI model. Default is `list()`.
- `PEAKVI_params`: A list of parameters for the PEAKVI model. Default is `list()`.
- `POISSONVI_params`: A list of parameters for the POISSONVI model. Default is `list()`.
- `train_params`: A list of parameters passed to the model `train()` method. Default is `list()`.
- `cores`: An integer setting the number of threads for `scVI`. Default is `1`.
