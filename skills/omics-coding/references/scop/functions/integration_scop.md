...
- Package: scop
- Language: R
- Function: `integration_scop`
- Source: local://scop/0.8.9/integration_scop
...
- `lisi_reduction`: Dimensional reduction used for LISI computation. Default is NULL , which uses DefaultReduction() from the integrated object.
...
- `metrics_reduction`: Reduction used for integration metric computation. Default is NULL , which uses DefaultReduction() from the integrated object.
...
- `metrics_tool_name`: Name of the tool entry used to store integration metrics. Default is NULL , which uses paste0(integration_method, "_metrics") .
...
- `scale_within_batch`: Whether to scale data within each batch. Only valid when the integration_method is one of "Uncorrected" , "Seurat" , "MNN" , "Harmony" , "BBKNN" , "CSS" , "ComBat" .
...
integration_scop R Documentation
