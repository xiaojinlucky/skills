...
这套 Skills 是在真实科研项目推进中逐步写出来的。科研卡住时，问题往往不是完全没有方法，而是文献、结果和可选的下一步太多，很难判断哪一项真正能改变主线；图表、分析和候选排序也容易在细节里失去原来的问题。
...
面对一批论文时，可以让 `literature-mining` 先做横向比较：不同名字的方法，实际完成的输入、操作和输出是否相近；再对关键论文做纵向拆解，把图、正文、图注和 Methods 串成“它怎样发现问题、怎样筛选和推进证据”的主线。只有逐篇核对输入、操作、筛选规则、输出语义和情境后，才把方法标为“已核验的功能等价”；否则只写“暂定功能相近/推断待核实”。

最后得到的是一张带证据状态的方向级工作地图，不是把论文结论拼成自己的答案，也不是自动生成因果链。需要把地图落到当前课题时，再由 `topic-convergence` 结合自己的数据、目标和资源筛选一个值得先做的问题；如果地图缺少对象、来源、情境或读出，会先选出一条最可核验、最可比的候选边，填一张最小验证卡，再决定是否回读文献或做预实验。
...
- [`literature-mining`](./skills/literature-mining/) 学习高质量论文怎样发现新问题、怎样建立证据，并从已知边界中寻找可核验的 GAP（尚未解决的问题）；不把别人结论直接搬成自己的课题。
...
- [`causality-rescue`](./skills/causality-rescue/) 校准相关、必要、充分、中介和直接作用等因果 claim（科学主张）的表达强度。
...
可以把仓库地址直接交给 Agent：`https://github.com/xiaojinlucky/skills`；也可以使用 `npx skills add https://github.com/xiaojinlucky/skills`。安装后按需要启用 `meta-research-hub` 和对应的核心 Skill。
...
只提供公开或已脱敏的材料。未发表课题、原始数据、个人信息和私人路径，不要直接上传到外部服务。
...
请先整理核心 claim（科学主张）和 Figure 的证据顺序，再按对结论的影响决定补充工作先后。
...
无需额外配置。它是面向研究思路的文本 Skill，可在支持该格式的 Agent 环境中使用；具体检索、数据分析、实验和写作执行仍由相应工具完成。

## 方法原则

- **先从自己的问题和证据出发。** 文献用于学习问题怎样被发现、证据怎样一步步建立，并划定已知边界；项目自己的候选和结论需要自己的证据支撑。
- **方法服务科学问题、论文主线和可发表性。** 优先选择高质量论文中常见、审稿人熟悉、适合当前数据并能回答问题的方法。包括 CNS（Cell、Nature、Science）正刊和大子刊在内，只要有合理的高质量论文先例，就可以作为基本背书，不让期刊标准变成束缚，也不让过度防御拖住课题。
