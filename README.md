...
这套 Skills 是在几个真实的单细胞和生信课题推进中逐步写出来的。科研卡住时，问题往往出在文献、结果和可选的下一步太多，很难判断哪一项能改变判断。一次轨迹分析中，两个 marker 图实际重复，第一版差异指标也没有回答轨迹问题，图中文字还发生重叠；这些问题都是逐项核对后才暴露出来。另一次从一组功能基因寻找上游 TF 时，固定的 TopN 过早排除了有独立证据的候选。
